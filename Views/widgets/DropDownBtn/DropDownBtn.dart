  ...
  Widget build(BuildContext context) {
    return DropdownButtonHideUnderline(
      child: DropdownButton(
        ...
        value: DropDownBase[widget.tag] == null
            ? dropdownValue
        ...
        onChanged: (String newVal) {
          if (int.parse(newVal) > widget.items.length - 1)
            newVal = widget.items.last.index.toString();
          setState(() {
            dropdownValue = newVal;
            DropDownBase.addAll(
                {widget.tag: widget.items[int.parse(dropdownValue)]});
...
class _DropDownBtnUnValState extends State<DropDownBtnUnVal> {
  ...
  void initState() {
    ...
    super.initState();
    if (widget.hideText == null) {
      DDBBase.changeDate(widget.tag, widget.items[0]);
    } else {
      DDBBase.changeDate(widget.tag, null);
    }
  }
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton(
          ...
          onChanged: (String newVal) {
            ...
            setState(() {
              DropDownBase.addAll(
                  {widget.tag: widget.items[int.parse(newVal)]});
